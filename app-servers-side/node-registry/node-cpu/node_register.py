        ...
        for monitoring_server in MONITORING_SERVERS:
            try:
                response = requests.post(
                    ...
                    json={
                            'labels': {'host_cpu': hostname, 'hostname': hostname, 'job': service_name, 'node_type': node_type}, 
                            'targets': [NODE_IP + ':' + str(port)]
                        }
                )
                if response.status_code == 200:
                    print(f"Service {service_name} in Node {HOSTNAME} sent to Monitoring service {monitoring_server}")
