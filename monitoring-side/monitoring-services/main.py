...
def add_node_config_file(new_node: Node):
    ...
    if isinstance(node_data, list):
        ...
        existed_node_services = set([node_service['targets'][0] for node_service in node_data])
...
def remove_node_config_file(node_ip: NodeIP):
    ...
    if node_data:
        existed_node_ip = set([node_service['targets'][0].split(':')[0] for node_service in node_data])
        if node_ip in existed_node_ip:
            new_node_data = [node_service for node_service in node_data if not any(node_ip in target for target in node_service.get('targets', []))]
